...
ax.hist(s1, bins=80, color=C1, alpha=0.9, edgecolor=BORDER, linewidth=0.8, zorder=2)
q1, q3 = np.percentile(s1, 25), np.percentile(s1, 75)
ax.axvspan(q1, q3, color=C1L, alpha=0.55, zorder=0)
ax.axvline(s1.median(), color=BORDER, lw=2, ls='--', zorder=3, label=f'Median {s1.median():.0f}m')
ax.axvline(s1.mean(),   color=C1,     lw=2, ls=':',  zorder=3, label=f'Mean {s1.mean():.0f}m')
ax.set_xlabel('Minutes', fontsize=9, fontweight='bold')
ax.set_ylabel('Games',   fontsize=9, fontweight='bold')
ax.set_title('Avg Playtime Forever\n(non-zero, n=8,010)', fontsize=10, fontweight='bold', pad=8)
ax.legend(fontsize=7.5, facecolor=BG_PANEL, edgecolor=BORDER, labelcolor=TEXT_C)
ax.text(0.98, 0.97, f'IQR={q3-q1:.0f}m\nOutliers 12.4%',
...
ax2.hist(s2, bins=80, color=C2, alpha=0.9, edgecolor=BORDER, linewidth=0.8, zorder=2)
q1, q3 = np.percentile(s2, 25), np.percentile(s2, 75)
ax2.axvspan(q1, q3, color=C2L, alpha=0.55, zorder=0)
ax2.axvline(s2.median(), color=BORDER, lw=2, ls='--', zorder=3, label=f'Median {s2.median():.0f}')
ax2.axvline(s2.mean(),   color=C2,     lw=2, ls=':',  zorder=3, label=f'Mean {s2.mean():.0f}')
ax2.set_xlabel('Peak CCU', fontsize=9, fontweight='bold')
ax2.set_ylabel('Games',    fontsize=9, fontweight='bold')
ax2.set_title('Peak CCU\n(non-zero, n=18,920)', fontsize=10, fontweight='bold', pad=8)
...
ax3.set_xlabel('Positive Review Rate (%)', fontsize=9, fontweight='bold')
ax3.set_ylabel('Games', fontsize=9, fontweight='bold')
ax3.set_title('Overall Positive Rate\n(valid reviews, n=53,199)', fontsize=10, fontweight='bold', pad=8)
...
fig3.suptitle('Steam Game Feature Distribution Analysis', fontsize=14,
...
ax.axvline(s1.mean(),   color=C1,     lw=2, ls=':',  zorder=3, label=f'Mean {s1.mean():.0f}m')
ax.set_xscale('log')
ax.set_xlabel('Minutes (log)', fontsize=9, fontweight='bold')
ax.set_ylabel('Games', fontsize=9, fontweight='bold')
ax.set_title('Avg Playtime Forever\n(non-zero, n=8,010)', fontsize=10, fontweight='bold', pad=8)
ax.legend(fontsize=7.5, facecolor=BG_PANEL, edgecolor=BORDER, labelcolor=TEXT_C)
ax.text(0.98, 0.97, f'IQR={q3-q1:.0f}m\nOutliers 12.4%',
...
ax2.set_xscale('log')
ax2.set_yscale('log')
ax2.set_xlabel('Peak CCU (log)', fontsize=9, fontweight='bold')
ax2.set_ylabel('Games (log)', fontsize=9, fontweight='bold')
ax2.set_title('Peak CCU\n(non-zero, n=18,920)', fontsize=10, fontweight='bold', pad=8)
ax2.legend(fontsize=7.5, facecolor=BG_PANEL, edgecolor=BORDER, labelcolor=TEXT_C)
ax2.text(0.98, 0.97, f'IQR={q3-q1:.0f}\nOutliers 16.8%',
        transform=ax2.transAxes, ha='right', va='top', fontsize=8,
...
fig2.suptitle('Steam Game Feature Distribution Analysis (Log Scale)', fontsize=14,
              fontweight='bold', color=TEXT_C, y=1.02,
              bbox=dict(boxstyle='square,pad=0.5', fc=CYAN_BTN, ec=BORDER, lw=2.5))
...
qscatter(fig1.add_subplot(gs1[0]), 'log_playtime', 'log_ccu',
         'log(平均遊玩時長)', 'log(峰值 CCU)', 'Playtime x CCU', '0.513')
qscatter(fig1.add_subplot(gs1[1]), 'log_ccu', 'pct_pos_total',
         'log(峰值 CCU)', '整體好評率 (%)', 'CCU x 好評率', '0.202')
qscatter(fig1.add_subplot(gs1[2]), 'log_playtime', 'pct_pos_total',
         'log(平均遊玩時長)', '整體好評率 (%)', 'Playtime x 好評率', '0.063')
